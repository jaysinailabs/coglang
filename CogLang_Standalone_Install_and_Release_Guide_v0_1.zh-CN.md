...
`1.1.3` 包发布已经完整走通过这条路径，后续 `1.1.x` 包级 patch
也应继续使用同一路径：

- stable `v1.1.x` tag 触发 Trusted Publishing
- GitHub Actions 在上传前构建并验证 wheel
- PyPI 收到了 wheel 与 sdist
- 该 tag 已有 GitHub Release，并使用包级 release notes
- 发布后验证从 PyPI 安装已发布的 `coglang[dev]==1.1.x`，并运行 `release-check` 与 `smoke`

---

## 7. package v1.1.x 之后还差什么

从当前状态走到更成熟的独立发布，下一层最现实的差距主要在：

1. **外部 host 或 consumer review**
   接受或链接第一个由 core runtime 仓库之外维护的 host 或 consumer。
2. **更稳的 Host Runtime Contract**
   让外部宿主可以更低成本接入。
3. **public asset 生成体验**
   降低 `_public_assets/` 手工镜像维护成本，同时不削弱包验证。
4. **更完整的 release automation**
   例如 release notes 生成和可重复的 GitHub Release 创建。
