...
def _formal_open_source_readiness_payload() -> dict[str, Any]:
    ...
    release_notes_path, _ = _resolve_project_artifact(
        "CogLang_Release_Notes_v1_1_6.md",
        "CogLang_Release_Notes_v1_1_6.md",
    )
...
def _manifest_payload() -> dict[str, Any]:
    ...
    readable_render_api_promotion_checklist_relpath = _resolve_project_artifact(
        "CogLang_Readable_Render_API_Promotion_Checklist_v0_1.md",
        "CogLang_Readable_Render_API_Promotion_Checklist_v0_1.md",
    )[1]
    ...
    return {
        ...
        "public_release_surface": {
            ...
            "project_docs": {
                ...
                "readable_render_api_promotion_checklist": docs[
...
def _release_check_payload() -> dict[str, Any]:
    ...
    readable_render_api_promotion_checklist_packaged = (
        "_public_assets/CogLang_Readable_Render_API_Promotion_Checklist_v0_1.md"
        in package_data
    )
    ...
    checks = [
        ...
        {
            "name": "readable_render_api_promotion_checklist",
            "ok": (
                readable_render_api_promotion_checklist_path.exists()
                and readable_render_api_promotion_checklist_packaged
            ),
            "detail": "readable render API promotion checklist + package data",
        },
        {
...
def _doctor_payload() -> dict[str, Any]:
    ...
    add_check("generated_artifact_dirs", True, _generated_artifact_dirs_detail())
    add_check("public_assets_mirror", True, _public_assets_mirror_doctor_detail())
...
def _generated_artifact_dirs_detail() -> str:
    root = _project_root()
    present = [
        name
    ...
    if not present:
        return "none"
    return "present: " + ", ".join(present) + "; generated by local build/test runs"


def _public_assets_mirror_doctor_detail() -> str:
    try:
        check = _public_assets_mirror_release_check()
    except FileNotFoundError:
        return "not applicable; public extract manifest not found"
...
def _pytest_args_include_basetemp(pytest_args: list[str]) -> bool:
    return any(arg == "--basetemp" or arg.startswith("--basetemp=") for arg in pytest_args)
